...
(
    ...
    HOME_LAT      NUMBER           NOT NULL, 
    HOME_LON      NUMBER           NOT NULL, 
    SESSION_ID    VARCHAR2(200)    NOT NULL, 
    ONLINE        VARCHAR2(20)     NOT NULL, 
    CONSTRAINT USER_TB_PK PRIMARY KEY (USR_SEQ)
)
/
...
COMMENT ON TABLE USER_TB IS '회원가입정보'
/

COMMENT ON COLUMN USER_TB.USR_SEQ IS '유저번호'
/

COMMENT ON COLUMN USER_TB.EMAIL IS '이메일'
/

COMMENT ON COLUMN USER_TB.ADDRESS1 IS '상위주소'
/

COMMENT ON COLUMN USER_TB.ADDRESS2 IS '하위주소'
/

COMMENT ON COLUMN USER_TB.SNS_ID IS '유저의 sns id'
/

COMMENT ON COLUMN USER_TB.PHONE IS '휴대폰 번호'
/

COMMENT ON COLUMN USER_TB.PASSWORD IS '유저비밀번호'
/

COMMENT ON COLUMN USER_TB.HOME_LAT IS '집위도'
/

COMMENT ON COLUMN USER_TB.HOME_LON IS '집경도'
/

COMMENT ON COLUMN USER_TB.SESSION_ID IS '세션ID'
/

COMMENT ON COLUMN USER_TB.ONLINE IS '온라인여부'
/



CREATE TABLE USR_LOCATION_TB
(
    USR_SEQ          NUMBER           NOT NULL, 
    DEAL_LAT         NUMBER           NULL, 
    DEAL_LON         NUMBER           NULL, 
    LOCATION_NAME    VARCHAR2(200)    NULL, 
    CONSTRAINT USR_LOCATION_TB_PK PRIMARY KEY (LOCATION_NAME)
)
/

COMMENT ON TABLE USR_LOCATION_TB IS '주소테이블'
/

COMMENT ON COLUMN USR_LOCATION_TB.USR_SEQ IS '유저번호'
/

COMMENT ON COLUMN USR_LOCATION_TB.DEAL_LAT IS '거래위도'
/

COMMENT ON COLUMN USR_LOCATION_TB.DEAL_LON IS '거래경도'
/

COMMENT ON COLUMN USR_LOCATION_TB.LOCATION_NAME IS '위치이름'
/

ALTER TABLE USR_LOCATION_TB
    ADD CONSTRAINT FK_USR_LOCATION_TB_USR_SEQ_USE FOREIGN KEY (USR_SEQ)
...
COMMENT ON COLUMN MESSAGE_TB.SENDER_CHK IS '보내는사람조회여부'
/
